...
class EncodingProvenance:
    """Frozen identity of the transformation that produced H, N, and B."""
    ...
    hidden_layer: int
    tau_threshold: float
    normalization: str = "row-minmax-v1"
    arithmetic_dtype: str = "float32"
    ...
    def __post_init__(self) -> None:
        ...
        if self.normalization != "row-minmax-v1":
            raise ValueError("unsupported normalization")
        if self.arithmetic_dtype != "float32":
            raise ValueError("unsupported arithmetic_dtype")
...
class SequencedBinaryAddress:
    ...
    @property
    def address_digest(self) -> str:
        digest = hashlib.sha256()
        digest.update(self.state_count.to_bytes(8, "big"))
...
def encode_episode(
    ...
    binary_state_sequence = threshold_state_vectors(
        ...
        encoding_provenance.tau_threshold,
    )
