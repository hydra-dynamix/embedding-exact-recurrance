...
def get_contextual_state_vectors(
    model: torch.nn.Module,
    input_ids: torch.Tensor,
    attention_mask: torch.Tensor,
) -> torch.Tensor:
    """
    Return the raw final-layer state for every attended contextual position.
    ...
    raw_contextual_state_vectors = final_layer_states[contextual_positions]

    return raw_contextual_state_vectors.float()
